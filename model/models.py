...
class CompGCNBase(BaseModel):
	def __init__(self, edge_index, edge_type, num_rel, params=None):
		...
		##pagerank
		self.use_pagerank = self.p.use_pagerank
		if self.use_pagerank:
			self.fc_pagerank = torch.nn.Linear(self.p.init_embed + 1, self.p.init_embed)
		##pagerank
	...
	def forward_base(self, sub, rel, drop1, drop2, pagerank_tensor=None):
		##pagerank
		# 기본 임베딩 가져오기
		ent_embed = self.init_embed

		# PageRank 반영 (옵션)
		if self.use_pagerank and pagerank_tensor is not None:
			pr_feat = pagerank_tensor.unsqueeze(1)  # shape: [num_ent, 1]
			ent_embed = torch.cat([ent_embed, pr_feat], dim=1)  # shape: [num_ent, init_dim + 1]
			ent_embed = self.fc_pagerank(ent_embed)  # shape: [num_ent, init_dim]
		##pagerank

		r	= self.init_rel if self.p.score_func != 'transe' else torch.cat([self.init_rel, -self.init_rel], dim=0)
		x, r	= self.conv1(ent_embed, self.edge_index, self.edge_type, rel_embed=r)
		x	= drop1(x)
...
class CompGCN_ConvE(CompGCNBase):
	...
	def forward(self, sub, rel):
		...
		sub_emb, rel_emb, all_ent	= self.forward_base(sub, rel, self.hidden_drop, self.feature_drop,pagerank_tensor=self.pagerank_tensor)
